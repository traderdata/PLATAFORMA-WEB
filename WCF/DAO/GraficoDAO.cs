...
namespace Traderdata.Server.App.TerminalWeb.DAO
{
    public class GraficoDAO : BaseDAO
    {
        ...
        /// <summary>
        /// Metodo que retorna o gráfico de acordo com o id
        /// </summary>
        /// <param name="ativo"></param>
        /// <param name="periodicidade"></param>
        /// <param name="idUsuario"></param>
        /// <returns></returns>
        public GraficoDTO ReturnById(int id)
        {
            ...
            try
            {
                ...
                hql.Append(" WHERE GR.GRAF_CD_GRAFICO = " + id );
        ...
        /// </summary>
        public GraficoDTO InserirGrafico(GraficoDTO graficoDTO)
        {
            StringBuilder hql = new StringBuilder();
            ...
            try
            {
                ...
                hql = new StringBuilder();
                hql.Append(" INSERT INTO GRAFICO (GRAF_NM_ATIVO, GRAF_VL_HEIGHT, GRAF_VL_LEFT, GRAF_VL_TOP, GRAF_VL_WIDTH, USUA_CD_USUARIO ,GRAF_VL_PERIODICIDADE, GRAF_DT_CADASTRO, WORK_CD_WORKSPACE) ");
                hql.Append(" VALUES (?ativo, ?height, ?left, ?top, ?width, ?usuario, ?periodicidade,?cadastro,?workspace)");
                ...
                using (MySql.Data.MySqlClient.MySqlCommand command = new MySql.Data.MySqlClient.MySqlCommand(hql.ToString(), writeConnection))
                {
                    ...
                    command.Parameters.AddWithValue("?height", graficoDTO.Height);
                    command.Parameters.AddWithValue("?left", graficoDTO.Left);
                    command.Parameters.AddWithValue("?top", graficoDTO.Top);
                    command.Parameters.AddWithValue("?usuario", graficoDTO.UsuarioId);
                    command.Parameters.AddWithValue("?width", graficoDTO.Width);
                    ...
                    if (graficoDTO.WorkspaceId != 0)
                        command.Parameters.AddWithValue("?workspace", graficoDTO.WorkspaceId);
                    else
                        command.Parameters.AddWithValue("?workspace", null);
                ...
                //Salvando os layouts
                foreach (LayoutDTO layout in graficoDTO.Layouts)
                {
                    using (LayoutDAO layoutDAO = new LayoutDAO(readConnection, writeConnection))
                    {
                        layout.GraficoId = graficoDTO.Id;
        ...
        /// <summary>
        /// Metodo que faz a edição do gráfico
        /// </summary>
        /// <param name="graficoDTO"></param>
        public void EditarGrafico(GraficoDTO graficoDTO)
        {
            try
            {
                //Excluindo o grafico
                this.ExcluiGrafico(graficoDTO);

                //inserindo o grafico
                this.InserirGrafico(graficoDTO);
