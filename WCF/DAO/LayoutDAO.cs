...
namespace Traderdata.Server.App.TerminalWeb.DAO
{
    public class LayoutDAO : BaseDAO
    {
        ...
        public void InsereLayout(LayoutDTO layout)
        {
            try
            {
                //salvando o template
                ...
                using (MySql.Data.MySqlClient.MySqlCommand command = new MySql.Data.MySqlClient.MySqlCommand(hql.ToString(), writeConnection))
                {
                    ...
                    //command.Parameters.AddWithValue("?cd", layout.Id);
        ...
        /// <summary>
        /// Metodo que exclui um layout
        /// </summary>
        /// <param name="layout"></param>
        public void ExcluiLayout(LayoutDTO layout)
        {
            try
            {
                //excluindo os paineis
                using (PainelDAO painelDAO = new PainelDAO(readConnection, writeConnection))
                {
                    painelDAO.ExcluiPainelByLayoutID(layout.Id);
                }
                //excluindo os indicadores
                using (IndicadorDAO indicadorDAO = new IndicadorDAO(readConnection, writeConnection))
                {
                    indicadorDAO.ExcluiIndicadorDeAcordoComLayoutId(layout.Id);
                }
                //excluindo os objetos
                using (ObjetoEstudoDAO objetoDAO = new ObjetoEstudoDAO(readConnection, writeConnection))
                {
                    objetoDAO.ExcluiObjetosDeAcordoComLayoutId(layout.Id);
                }

                //fazer a exclusao do layout em si
                //apagando o template
                StringBuilder hql = new StringBuilder();
                hql.Append(" DELETE FROM ");
                hql.Append(" LAYOUT ");
                hql.Append(" WHERE LAYO_CD_LAYOUT = " + layout.Id);
